            ...
            <div class="traffic_information col-md-offset-1 col-md-10">
                ...
                <table class="table table-striped">
                    ...
                    <tr>
                        ...
                        <td><fmt:formatDate value="${trainTicket.goOff}" pattern="MM月dd日 HH:mm" /></td>
                        <td><fmt:formatDate value="${trainTicket.arriveTime}" pattern="MM月dd日 HH:mm" /></td>
                        <td>${trainTicket.startStation}</td>
                        <td>${trainTicket.desStation}</td>
                        <td><span class="glyphicong glyphicon-yen"></span>${trainSeats[0].piece}</td>
                        <td>${seatType}</td>
                        <td>
                            <select id="tsId">
                                <c:forEach items="${trainSeats}" var="seat">
                                    <option value="${seat.tsId}">${seat.seatNo}</option>
                                </c:forEach>
                            </select>
                        </td>
                    </tr>
        ...
        <div class="row">
            <div class="traffic_passenger col-md-offset-1 col-md-10">
                ...
                <div class="col-md-offset-5 col-md-2" style="margin-top: 50px; margin-bottom: 50px">
                    <c:if test="${tbId == null}">
                        <button id="add_trainBill" class="btn btn-default btn-block">提交订单</button>
                    </c:if>
                    <c:if test="${tbId != null}">
                        <input value="${tbId}" id="change_tbId" type="hidden" />
                        <button id="change_trainBill" class="btn btn-default btn-block">改签</button>
                    </c:if>
                </div>
            </div>
        </div>
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data.err!=null){
                        ...
                        layer.msg(data.msg, {icon: 1});
                        setTimeout(function (){window.location.href='http://localhost:8080/userTrainBill'},1000);
                    }
                }
            });
        });

        $('#change_trainBill').click(function () {
            var data = {
                "pId":$('#pId').val(),
                "tsId":$('#tsId').val(),
                "type":$('#type').val()
                ...
                success: function(data) {
                    if(data.err!=null){
                        layer.msg(data.err, {icon: 2});
                         ...
                         $.ajax({
                            url: '${billURL}/trainbill/' + $('#change_tbId').val(),
                            type: 'DELETE',
                            xhrFields: {
                                withCredentials: true
                            },
