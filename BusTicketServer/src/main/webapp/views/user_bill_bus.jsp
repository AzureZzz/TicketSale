                ...
                <div class="layui-tab">
                    ...
                    <div class="layui-tab-content">
                        <div class="layui-tab-item layui-show">
                            <table id="unFinish_bus" lay-filter="unfinish_bus"></table>
                        </div>
                        <div class="layui-tab-item">
                            <table id="finish_bus" lay-filter="finish_bus"></table>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    layui.use(['element','table','layer','form'], function() {
        ...
        table.render({
            ...
            ,cols: [[
                {title: '车次信息',width:220, unresize:true,align:'center',templet: function(d){
                        return new Date(d.busSeat.busNum.goOff).Format("yyyy-MM-dd HH:mm")+'开<br>' +
                            d.busSeat.busNum.numNo+'次<br>'+d.busSeat.busNum.startStation +'-' +
                            d.busSeat.busNum.desStation;
                    }}
                ...
                ,{title: '票款金额',unresize:true,align:'center',templet: function(d){
                        if(d.billType == '成人'){
                            return d.billType+'票<br>' +
                                d.busSeat.busNum.piece+'元';
                        }else{
                            return d.billType+'票<br>' +
                                d.busSeat.busNum.piece/2+'元';
                        }
                    }}
        ...
        table.on('tool(finish_bus)', function(obj){
            var data = obj.data;
            if(obj.event === 'del'){
                layer.confirm('确认退票？', function(index){
                    $.ajax({
                        url:'${billURL}/busbill/' + data.bbId,
                        type:'DELETE',
                        dataType:'json',
                ...
                window.location.href='/changeBusTicket?bbId='+data.bbId;
            }
        });

        table.on('tool(unfinish_bus)', function(obj) {
            var data = obj.data;
            if (obj.event === 'del') {
                ...
                if(data.billType == '成人'){
                    $('#pay_piece').val(data.busSeat.busNum.piece);
                }else{
                    $('#pay_piece').val(data.busSeat.busNum.piece/2);
                }
                $('#bbId').val(data.bbId);
                layer.open({
                    type: 1
...
<div id="pay_alert" style="height: 190px;padding: 20px;display: none">
    <form class="layui-form">
        <input id="bbId" class="hidden" name="bbId" value="">
