...
public class PassengerController {
    ...
    @Autowired
    private LayUITableUtils<Passenger> layUITableUtils;

    @AuthLogin
    @ResponseBody
    @RequestMapping(value = "passengers",method = RequestMethod.GET)
    public Map<String,Object> getUserPassengers(@RequestParam int page,@RequestParam int limit,
                                                HttpServletRequest request){
        Long uId = ServiceUtils.getUid(request);
        Map<String,Object> map = null;
        try {
            List<Passenger> passengers = passengerService.getUserPassenger(uId);
            map = layUITableUtils.getLayUIMap(passengers);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return map;
