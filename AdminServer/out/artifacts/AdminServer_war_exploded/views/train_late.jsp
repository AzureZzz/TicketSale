                    ...
                    <div class="layui-col-md12">
                        <div class="layui-card">
                            ...
                            <div class="layui-card-body">
                                <div class="layui-btn-group demoTable float-left">
                                    <button class="layui-btn" data-type="delAll">全部删除</button>
                                </div>
                                <div class="layui-btn-group demoTable">
                                    <button class="layui-btn" data-type="add_late">新增晚点</button>
                                </div>
...
<div id="new_late" style="height: 250px;padding: 30px;display: none">
    <form class="layui-form" id="new_form">
        ...
        <div class="layui-form-item">
            <div class="layui-inline">
                ...
                <div class="layui-input-block"  style="width: 160px;">
                    <input id="trainNo" name="trainNo" type="text" required lay-verify="" autocomplete="off"
                           class="layui-input">
                </div>
            </div>
            <div class="layui-inline">
        ...
        <div class="layui-form-item">
            <div class="layui-inline">
                <label class="layui-form-label" style="width: 100px;">晚点类型</label>
                <div class="layui-input-block" style="width: 160px;">
                    <select name="type" lay-filter="type">
                        <option value="1" selected="">出发站晚点</option>
                        <option value="0">到达站晚点</option>
                    </select>
                </div>
            </div>
            <div class="layui-inline">
                <label class="layui-form-label" style="width: 100px;">新时间</label>
                <div class="layui-input-block">
                    <input id="newTime" name="newTime" type="text" required lay-verify="" autocomplete="off"
                           class="layui-input">
                </div>
            </div>
        </div>
        <div class="layui-form-item float-right">
            <div class="layui-input-block">
                <input type="button" lay-submit="" lay-filter="add_late" class="layui-btn" value="确认"/>
...
<div id="modify_late" style="height: 180px;padding: 30px;display: none">
    <form class="layui-form" id="new_form1">
        <input type="hidden" id="ltId" name="ltId" value="">
        <div class="layui-form-item">
            <div class="layui-inline">
                <label class="layui-form-label" style="width: 100px;">晚点类型</label>
                <div class="layui-input-block" style="width: 160px;">
            ...
            <div class="layui-inline">
                <label class="layui-form-label" style="width: 100px;">新时间</label>
                <div class="layui-input-block">
                    <input id="newTime1" name="newTime" type="text" required lay-verify="" autocomplete="off"
                           class="layui-input">
                </div>
            </div>
        </div>
        <div class="layui-form-item float-right">
            <div class="layui-input-block">
                <input type="button" lay-submit="" lay-filter="modify_late" class="layui-btn" value="确认"/>
                <input type="button" class="layui-btn layui-btn-danger close_frame" value="取消"/>
            </div>
        </div>
    </form>
...
<script>
    layui.use(['table','element','form','laydate'], function(){
        ...
        form.on('submit(add_late)', function (datas) {
            var data={
        ...
        form.on('submit(modify_late)', function (datas) {
            var data={
                "ltId":datas.field.ltId,
                "newTime":datas.field.newTime,
                "type":datas.field.type
            };
            $.ajax({
        ...
        table.render({
            ...
            ,cols: [[
                {type: 'checkbox'}
                ,{field:'ltId', title:'车次', width:100, unresize: true, sort: true,templet: function(d){
                    return 'Z6';
                }}
                ,{field:'startStation', title:'出发站', width:100, unresize: true, sort: true,templet: function(d){
                        return d.trainTicket.startStation;
                }}
                ,{field:'desStation', title:'到达站', width:100, unresize: true, sort: true,templet: function(d){
                        return d.trainTicket.desStation;
                }}
                ,{field:'trainTicket', title:'原出发时间',templet: function(d){
                        return new Date(d.trainTicket.goOff).Format("MM月dd日 HH:mm");
                }}
                ,{field:'newTime', title:'现出发时间',templet: function(d){
                        return new Date(d.newTime).Format("MM月dd日 HH:mm");
                }}
                ,{field:'type', title: '晚点类型', minWidth:120, sort: true,templet: function(d){
                    return d.type == true ? '出发站晚点':'到达站晚点';
                }}
        ...
        table.on('tool(late_filter)', function(obj){
            var data = obj.data;
            if(obj.event === 'del'){
                layer.confirm('是否删除该晚点信息？', function(index){
                    $.ajax({
                        url: "/latetrain/"+data.ltId,
                        type: 'DELETE',
                        contentType: 'application/json',
                        dataType:'json',
                        success: function(res){
                ...
                $('#ltId').val(data.ltId);
                $('#type').val(data.type == true ? 1:0);
                $('#newTime1').val(new Date(data.newTime).Format("yyyy-MM-dd HH:mm:ss"));
                layer.open({
                    type: 1
                    ,title: ['修改火车晚点信息']
                    ,closeBtn: false
        ...
        var $ = layui.$, active = {
            delAll: function(){ //获取选中数据
                var checkStatus = table.checkStatus('late_table')
                    ,datas = checkStatus.data;
                var ids = new Array(datas.length);
                datas.forEach(function(value,index,array){
                    ids[index] = value.ltId;
                });
                var data = {
                    "ids":ids
                };
